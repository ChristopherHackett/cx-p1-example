...
if (!cX.library) {
    cX.library = {
        ...
        _findRealReferrer: function() {
            var realReferrer = (typeof this.documentReferrer === 'string' ? this.documentReferrer : document.referrer) || '';
        ...
        getUserSegmentIds: function(args, requestObject) {
            ...
            function readQueryCache() {
                ...
                try {
                    // Try the new format first (<persistedQueryId>_<time>_<ids>/<persistedQueryId>:<time>:<ids>|.. etc)
                    var newFormatQueryCache = cX.getCookie('_cX_segmentInfo');
                    if (newFormatQueryCache) {
                        ...
                        // Try reading the dld format query cache cookies
                        requestTime = parseInt(cX.getCookie('_cX_segmentTime') || '0');
                        var oldFormatSegmentIds = cX.getCookie('_cX_segmentIds');
                        segmentIds = oldFormatSegmentIds ? oldFormatSegmentIds.split(',') : (args.defaultValue || []);
                        // Assume the query was stored for the same persistedQueryId as this call.
                        // That will be correct in most situations, and correct itself over time for the others.
                        cachedQueries[args.persistedQueryId] = { requestTime: requestTime, segmentIds: segmentIds };
                    }
                } catch (e) { }
                // Cleanup the old format cache cookies
                // try { cX.deleteCookie('_cX_segmentTime'); } catch (e) { }
                // try { cX.deleteCookie('_cX_segmentIds'); } catch (e) { }
                return cachedQueries;
            }

            function writeQueryCache(cachedQueries) {
                // LRU
                var cachedQueriesList = [];
                for (var persistedQueryId in cachedQueries) {
                    if (cachedQueries.hasOwnProperty(persistedQueryId)) {
                        var cachedQuery = cachedQueries[persistedQueryId];
                ...
                for (var i = 0; i < 5 && i < cachedQueriesList.length; i++) {
                    var cachedQuery = cachedQueriesList[i];
            ...
            if (isNaN(cachedSegmentTime) || (cachedSegmentTime < now - maxAge) || cachedSegmentTime > now) {
                ...
                this.jsonpRequest(url, function(data) {
                    ...
                    if (cX.isArray(segments)) {
                        ...
                        (cachedQueries[args.persistedQueryId] || {}).segmentIds = segments;
                        writeQueryCache(cachedQueries);
                    }
                    if (args.callback) {
        ...
        sync: function() {
            ...
            cX.Array.forEach(arguments, function(provider) {
                try {
                    if (this._wasSyncedWithExternalParty(provider)) {
                        return;
                    }
                } catch (e) {}
                switch (provider) {
                    case 'xl8': new Image().src = '//loadm.exelator.com/load/?p=204&g=820&j=0'; break;
                    case 'neu': new Image().src = '//adadvisor.net/adscores/g.pixel?sid=9212271498'; break;
                    case 'p161': new Image().src = '//ads.creative-serving.com/cm?redir=http' + (secure ? 's' : '') + '://csyn.cxense.com/pfm.png?uuid=${UUID}'; break;
                    case 'liveramp': new Image().src = '//idsync.rlcdn.com/421746.gif?partner_uid=' + cX.getUserId(); break;
                    case 'liveramp2': new Image().src = '//csyn-r.cxense.com/?partnerId=lr2&redir=http%3A%2F%2Fidsync.rlcdn.com%2F421746.gif%3Fpartner_uid%3D%24%7BUUID%7D'; break;
                }
            }, this);
        },

        _wasSyncedWithExternalParty: function(provider) {
            var key = 'cx_partner';
            ...
            var addProviderToExclusionSyncList = function(provider, storageItem) {
                if (storageItem === null || typeof storageItem === 'undefined') {
                    storageItem = provider;
                } else {
                    storageItem = storageItem + ',' + provider;
                }
                setStorageItem.call(this, storageItem);
            };
            var storageItem = getStorageItem.call(this);
            if (storageItem === null || typeof storageItem === 'undefined') {
                addProviderToExclusionSyncList.call(this, provider, storageItem);
                return false;
            } else {
